...
class Metrics(Callback):
    def __init__(self, platform, val_data = None, batch_size = 20):
...
class LLayer(Layer):
    ...
    def build(self, input_shape, mask=None):
        self.Wl = K.variable(self.init((self.latent_dim, self.latent_dim)), name="Wl")
...
class Defend():
    ...
    def _build_model(self, n_classes=2, embedding_dim=100, embeddings_path=False, aff_dim=80):
        ...
        l_lstm_sent = Bidirectional(GRU(100, return_sequences=True), name='sentence_lstm')(Reshape((50, 200))(content_encoder))
    ...
    def load_weights(self, saved_model_dir, saved_model_filename):
        with CustomObjectScope({'AttLayer': AttLayer, 'LLayer': LLayer}):
            ...
            self.news_sequence_encoder = self.model.get_layer('time_distributed')
    ...
    def train(self, train_x, train_y, train_c, val_c, val_x, val_y, platform='',
        ...
        self.metrics = Metrics(platform, [encoded_val_c, encoded_val_x, val_y], batch_size)
    ...
    def activation_maps(self, news_article_sentence_list, news_article_comment_list, websafe=False):
        ...
        sentence_rep_trans = np.transpose(sentence_level_weights, axes=(0, 2, 1))
